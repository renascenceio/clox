...
export const runJavaScriptTool = tool({
  description: [
    ...
    'the result and stdout.',
    '',
    'USE THIS ONLY when JavaScript is the right tool for a CONCRETE',
    'computation: exact arithmetic, simple data transforms, regex testing,',
    'parsing JSON the user pasted, validating a date, computing a hash, or',
    'any time you would otherwise be tempted to "calculate by hand" and',
    'risk an off-by-one.',
    '',
    'DO NOT call this tool as a "planning" or "thinking out loud"',
    'scratchpad. The sandbox has no side effects, so a snippet that just',
    'writes a comment like `// plan the deck` returns `undefined` and',
    'wastes a turn. If you need to plan, write the plan as prose in your',
    'reply — do NOT spend a tool call on it.',
    '',
    'DO NOT use this tool to build documents (PPTX, PDF, XLSX, DOCX).',
    'For documents, emit the appropriate fenced-block format the host',
    'understands (```pptx, ```pdf, etc.), or call the python tool when it',
    'is available in this turn. Never use run_javascript as a stepping',
    'stone toward those deliverables.',
    '',
    'The sandbox has NO network, NO file system, NO module imports, and a',
    '1 second timeout. Available: Math, JSON, Date, Number, String, Array,',
    'Object, Map, Set, Symbol, BigInt, Promise, and a global `print(...)`',
    'helper that appends to stdout. The last expression is returned as',
    '`result`.',
  ].join(' '),
  parameters: z.object({
    code: z
