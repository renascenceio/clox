...
export function makePythonTool(chatId: string, onProgress?: SandboxProgressCallback) {
  return tool({
    ...
    description: [
      ...
      '  - "Continue" only works when state is on disk. If your in-memory',
      '    pptx/docx/xlsx object truncates, the next turn has nothing',
      '    to resume from. With on-disk saves, the next turn just opens',
      '    /mnt/user-data/outputs/<file> and appends the next unit.',
      '',
