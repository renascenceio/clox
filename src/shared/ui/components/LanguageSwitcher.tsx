...
export default function LanguageSwitcher() {
  ...
  return (
    <button
      ...
      className="flex items-center gap-0.5 p-0.5 rounded-full bg-surface-tertiary dark:bg-surface border border-separator/50 transition-all hover:border-mint/40 dark:hover:border-teal/40"
      aria-label="Toggle language"
    >
      <span className={`px-2.5 py-1 rounded-full text-[11px] font-bold transition-all ${
        currentLang === 'en'
          ? 'bg-mint dark:bg-teal text-white shadow-sm'
